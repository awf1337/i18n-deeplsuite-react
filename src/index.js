...
const { generateI18nFiles, generateI18FilesTs } = require('./i18n-utils');
...
const isESlint = process.argv.includes('lint');

if (isTypeScript) {
  process.argv = process.argv.filter(arg => arg !== 'ts');
}
...
const allFiles = walk(path.join(path.resolve(inputDir), sourceDir), isTypeScript);
